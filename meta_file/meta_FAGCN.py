...
def trial(hyperpm):
    ...
    cmd = 'python train_GCN.py --net FAGCN'
    for k in hyperpm:
        v = hyperpm[k]
        cmd += ' --' + k
    ...
    if score < min_y:
        ...
        f= open("./FAGCN_log/logger-{}-2.txt".format(args.dataset),"a+")
        f.write('>>>>>>>>>> min val now=%5.2f%% @ %s\n' % (-min_y*100, min_c))
        f.close()
    if tst_score < min_tst:
        min_tst, min_tst_c = tst_score, cmd
        f= open("./FAGCN_log/logger-{}.txt".format(args.dataset),"a+")
...
parser.add_argument('--RPMAX', type=int, default=15,
                    help='the number of test')
args = parser.parse_args()


space = {'lr': hyperopt.hp.choice('lr', [0.01, 0.05, 0.001, 0.005, 0.002]),
         'weight_decay': hyperopt.hp.choice('weight_decay', [0.00025, 0.0001, 0.001, 0.0025, 0.005, 5e-4, 0.0]),
         'hidden': hyperopt.hp.choice('hidden', [8, 16, 32, 64, 128]),
         'feat_drop': hyperopt.hp.choice('feat_drop', [0, 0.2, 0.4, 0.6, 0.8]),
         #'attn_drop': hyperopt.hp.quniform('attn_drop', 0, 0.9, 0.1),
         'eps': hyperopt.hp.quniform('eps', 0.1, 1.0, 0.1),
