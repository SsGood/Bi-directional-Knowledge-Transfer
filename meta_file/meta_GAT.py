...
def trial(hyperpm):
    ...
    cmd = 'python train_GCN.py --net GAT'
    for k in hyperpm:
        v = hyperpm[k]
        cmd += ' --' + k
    ...
    if score < min_y:
        ...
        f= open("./GAT_log/logger-{}-2.txt".format(args.dataset),"a+")
        f.write('>>>>>>>>>> min val now=%5.2f%% @ %s\n' % (-min_y*100, min_c))
        f.close()
    if tst_score < min_tst:
        min_tst, min_tst_c = tst_score, cmd
        f= open("./GAT_log/logger-{}.txt".format(args.dataset),"a+")
...
space = {'lr': hyperopt.hp.choice('lr', [0.01, 0.05, 0.001, 0.005, 0.002]),
         ...
         'layer_num': hyperopt.hp.choice('layer_num',[1,2,3]),
