...
def trial(hyperpm):
    ...
    cmd = 'python train_GNN_auto_bi_w_noise.py --net GCNII'
    for k in hyperpm:
        v = hyperpm[k]
        cmd += ' --' + k
    ...
    if score < min_y:
        ...
        f= open("./GCNII_log/logger-{}-2.txt".format(args.dataset),"a+")
        f.write('>>>>>>>>>> min val now=%5.2f%% @ %s\n' % (-min_y*100, min_c))
        f.close()
    if tst_score < min_tst:
        min_tst, min_tst_c = tst_score, cmd
        f= open("./GCNII_log/logger-{}.txt".format(args.dataset),"a+")
...
space = {'lr': hyperopt.hp.choice('lr', [0.01, 0.05, 0.001, 0.005, 0.002]),
         'weight_decay': hyperopt.hp.choice('weight_decay', [0.0005, 0.00025, 0.0001, 0.001, 0.0025, 0.005, 5e-4, 5e-6, 0.0]),
         'hidden': hyperopt.hp.choice('hidden', [8, 16, 32, 64, 128, 256]),
         'feat_drop': hyperopt.hp.quniform('feat_drop', 0, 0.9, 0.1),
         'lamda': hyperopt.hp.quniform('lamda', 0, 0.9, 0.1),
         #'attn_drop': hyperopt.hp.quniform('attn_drop', 0, 0.9, 0.1),
         #'slope': hyperopt.hp.quniform('slope', 0.1, 0.5, 0.1),
         'layer_num': hyperopt.hp.choice('layer_num', [1, 2, 4, 8, 16, 32, 64]),
