...
def trial(hyperpm):
    ...
    cmd = 'python train_GCN.py --net GraphSAGE'
    for k in hyperpm:
        v = hyperpm[k]
        cmd += ' --' + k
    ...
    if score < min_y:
        ...
        f= open("./SAGE_log/logger-{}-2.txt".format(args.dataset),"a+")
        f.write('>>>>>>>>>> min val now=%5.2f%% @ %s\n' % (-min_y*100, min_c))
        f.close()
    if tst_score < min_tst:
        min_tst, min_tst_c = tst_score, cmd
        f= open("./SAGE_log/logger-{}.txt".format(args.dataset),"a+")
...
parser.add_argument('--RPMAX', type=int, default=25,
