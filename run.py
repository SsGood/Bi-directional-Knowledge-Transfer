...
def cmd_run(cmd, hyperpm, pm, args):
    ...
    filename = f'results/compare_{gnn}_mlp_or_and.csv'
    with open(f"{filename}", 'a+') as write_obj:
        write_obj.write(f"{gnn}," + f"{dataset}," + f"{union_acc} +- {union_acc_std}," + f"{xor_acc} +- {xor_acc_std}," + f"{gnn_acc} +- {gnn_acc_std}," + f"{mlp_acc} +- {mlp_acc_std}\n")
...
#     for dataset in ['cora', 'citeseer', 'pubmed', 'computers', 'photo']:
    for dataset in ['cora', 'citeseer', 'pubmed', 'computers', 'photo']:
        ...
        pm = [args.net, args.dataset, args.iter_num, args.dis_weight, args.gen_weight]

        cmd = f'python compare_gnn_mlp.py --result_path results --iter_num {args.iter_num} --dis_weight {args.dis_weight} --gen_weight {args.gen_weight} --dataset {args.dataset}'
